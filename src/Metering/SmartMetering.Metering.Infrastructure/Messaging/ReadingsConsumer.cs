    ...
    ILogger<ReadingsConsumer> logger) : BackgroundService
{
    ...
    private const string QueueName = "metering.readings";
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        logger.LogInformation("ReadingsConsumer listening on '{Queue}'", QueueName);
        await Task.Delay(Timeout.Infinite, stoppingToken); // giữ service sống
    }
    ...
    private async Task OnReceivedAsync(object sender, BasicDeliverEventArgs ea)
    {
        try
        {
            ...
            if (evt is not null)
            {
                ...
                var handler = scope.ServiceProvider.GetRequiredService<StoreReadingHandler>();
        ...
        {
            logger.LogError(ex, "Failed to process reading");
